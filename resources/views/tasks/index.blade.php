<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <button @click="$dispatch('open-modal', 'create-task')" class="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold py-2.5 px-5 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 text-sm flex items-center gap-2">
        ...
        <div class="max-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($tasks->isEmpty())
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl p-12 text-center">
                    ...
                    <button @click="$dispatch('open-modal', 'create-task')" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition text-sm font-medium">+ Nueva Tarea</button>
                </div>
            @else
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button @click="$dispatch('open-modal', 'edit-task-{{ $task->id }}')" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                        ...
                                        <button @click="$dispatch('open-modal', 'delete-task-{{ $task->id }}')" class="text-red-500 hover:text-red-700">
    ...
    {{-- Modal: Crear tarea --}}
    <div x-data="{ open: false }" x-show="open" @open-modal.window="if ($event.detail === 'create-task') open = true" @keydown.escape="open = false" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div x-show="open" x-transition.opacity class="fixed inset-0 bg-black/50" @click="open = false"></div>
        <div x-show="open" x-transition class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10">
            ...
            <form action="{{ route('tasks.store') }}" method="POST">
                ...
                <div class="flex justify-end gap-3 mt-6">
                    <button type="button" @click="open = false" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium">Cancelar</button>
                    <button type="submit" class="px-5 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-sm font-semibold rounded-lg hover:from-blue-600 hover:to-indigo-700 shadow-md hover:shadow-lg transition-all">Crear Tarea</button>
                </div>
            </form>
        </div>
    </div>
    ...
    {{-- Modal: Editar tarea --}}
    @foreach ($tasks as $task)
    <div x-data="{ open: false }" x-show="open" @open-modal.window="if ($event.detail === 'edit-task-{{ $task->id }}') open = true" @keydown.escape="open = false" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div x-show="open" x-transition.opacity class="fixed inset-0 bg-black/50" @click="open = false"></div>
        <div x-show="open" x-transition class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10">
            <div class="flex items-center justify-between mb-4">
            ...
            <form action="{{ route('tasks.update', $task) }}" method="POST">
                ...
                <div class="flex justify-end gap-3 mt-6">
                    <button type="button" @click="open = false" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium">Cancelar</button>
                    <button type="submit" class="px-5 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-sm font-semibold rounded-lg hover:from-indigo-600 hover:to-purple-700 shadow-md hover:shadow-lg transition-all">Guardar Cambios</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-data="{ open: false }" x-show="open" @open-modal.window="if ($event.detail === 'delete-task-{{ $task->id }}') open = true" @keydown.escape="open = false" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div x-show="open" x-transition.opacity class="fixed inset-0 bg-black/50" @click="open = false"></div>
        <div x-show="open" x-transition class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10 text-center">
            ...
            <form action="{{ route('tasks.destroy', $task) }}" method="POST">
                ...
                <div class="flex justify-center gap-3">
                    <button type="button" @click="open = false" class="px-5 py-2 text-sm text-gray-600 hover:text-gray-900 font-medium border border-gray-300 rounded-lg hover:bg-gray-50">Cancelar</button>
                    <button type="submit" class="px-5 py-2 bg-red-600 text-white text-sm font-semibold rounded-lg hover:bg-red-700 shadow-md transition-all">Eliminar</button>
                </div>
            </form>
        </div>
    </div>
